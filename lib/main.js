  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(atom.config.get('linter-ansible.yamlLintExecutablePath'), args, {cwd: require('path').dirname(file), ignoreExitCode: true, timeout: atom.config.get('linter-yaml.timeout') * 1000}).then(output => {
